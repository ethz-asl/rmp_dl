...
def sun3d_home_at():
    ...
    planner3dvis = Planner3dVisComparison(worldgen, distancefield=False, voxel_grid=True, start=False,
                                          voxel_grid_loc=worldgen.get_goal() + np.array([2.0, .0, .0]), 
                                          voxel_grid_size=np.array([3.0, 1.6, 1.0]))
    ...
    for i, start in enumerate(starts):
        ...
        PlannerComparison3dVisFactory.add_rrt_planner(planner3dvis, color=[43. / 255., 1, 0], name=f"rrt{i}")
