...
def main():
    ...
    worldgen_callable = lambda num: ProbabilisticWorldgenFactory.sphere_box_world(num, seed=1215, goal=np.array([7.,6.,8.]))

    world_builder = WorldBuilderActor(worldgen_callable, num_obstacles=200)
    md = MovieDirector()

    md.register_actor(world_builder)

    world_mesh = Plot3D.get_world_geometry(worldgen_callable(0))
    goal_geometry = Plot3D.get_sphere_geometry(worldgen_callable(0).get_goal(), color=np.array([0, 1, 0]), radius=0.4)
    ...
    directory = rmp_io.resolve_directory("media/videos/presentation/03-world_builder")
    # directory = None

    Studio.do_run_and_make_video(md, output_directory=directory, filename="03-world_builder.mp4", delete_images=False)
