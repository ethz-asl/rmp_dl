/** From: https://github.com/ethz-asl/mav_local_avoidance/blob/master/local_trajectory_opt/include/local_trajectory_opt/chomp/chomp_optimizer.h */
#ifndef LOCAL_TRAJECTORY_OPT_CHOMP_CHOMP_OPTIMIZER_H_
#define LOCAL_TRAJECTORY_OPT_CHOMP_CHOMP_OPTIMIZER_H_
 ...
 public:
  ChompParameters()
      : D(3),
        ...
        w_collision(5.0),
        epsilon(0.1),
        lambda(100),  // Suggested value: 1000?
        rel_tol(1e-5),
        max_iter(1000),
...
struct ChompTrajectory {
 public:
  ...
  Eigen::VectorXd getAcceleration(int row) const {
    if (row == 0) {
      // Forward differentiation at first points
      return N * N * (trajectory.row(row + 2) - 2 * trajectory.row(row + 1) +
                      trajectory.row(row));
    } else if (row == trajectory.rows() - 1) {
      // Backwards at last point
      return N * N * (trajectory.row(row - 2) - 2 * trajectory.row(row - 1) +
      ...
      // Middle everywhere else
      return N * N * (trajectory.row(row + 1) - 2 * trajectory.row(row) +
...
class CollisionFunc : public ChompCostFunc {
 public:
  ...
  void setDistanceGradientFunction(
      const DistanceGradientFunctionType& function) {
