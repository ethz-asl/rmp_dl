...
LidarCloudNode::LidarCloudNode()
  ...
  params.raycastingCudaPolicyParameters.N_sqrt = 182;
  params.raycastingCudaPolicyParameters.r = 5;
  params.raycastingCudaPolicyParameters.c_softmax_obstacle = 5;
  params.raycastingCudaPolicyParameters.epsilon_damp = 0.001;
  params.raycastingCudaPolicyParameters.eta_damp = 0.2;
  params.raycastingCudaPolicyParameters.v_damp = 20;
  params.raycastingCudaPolicyParameters.eta_rep = 30;
  params.raycastingCudaPolicyParameters.v_rep = 0.1;
  params.raycastingCudaPolicyParameters.metric = true;

  ray_policy_ = std::make_shared<RayPolicy>(params.raycastingCudaPolicyParameters);
  target_policy_ = std::make_shared<TargetPolicy>(
      Eigen::Vector3d(1.0, 1.0, 1.0),
      Eigen::Matrix3d::Identity() * params.targetParameters.metric, 
      params.targetParameters.alpha, params.targetParameters.beta, params.targetParameters.c_softmax);
...
void LidarCloudNode::configCallback(rmpcpp_planner::LidarPlannerConfig &config,
                                    uint32_t level) {
  params.targetParameters.alpha = config.alpha_goal;
  params.targetParameters.beta = config.beta_goal;
  params.targetParameters.c_softmax = config.gamma_goal;

  params.raycastingCudaPolicyParameters.eta_damp = config.eta_damp;
  params.raycastingCudaPolicyParameters.v_damp = config.v_damp;
  params.raycastingCudaPolicyParameters.epsilon_damp = config.epsilon_damp;
  params.raycastingCudaPolicyParameters.metric = config.metric;

  params.raycastingCudaPolicyParameters.eta_rep = config.eta_rep;
  params.raycastingCudaPolicyParameters.v_rep = config.v_rep;
  params.raycastingCudaPolicyParameters.lin_rep = config.lin_rep;
  params.raycastingCudaPolicyParameters.r = config.r;
  params.raycastingCudaPolicyParameters.c_softmax_obstacle = config.c_softmax_obstacle;
  params.targetParameters.metric = config.metric_goal;
  ray_policy_->updateParams(params.raycastingCudaPolicyParameters);
  target_policy_->updateParams(params.targetParameters.alpha, params.targetParameters.beta,
                               params.targetParameters.c_softmax);
}

void LidarCloudNode::goalCallback(const geometry_msgs::Point &pt) {
  goal_W_ = Eigen::Vector3d(pt.x, pt.y, pt.z);
  target_policy_ = std::make_shared<TargetPolicy>(
      goal_W_, Eigen::Matrix3d::Identity() * params.targetParameters.metric,
      params.targetParameters.alpha, params.targetParameters.beta, params.targetParameters.c_softmax);
}
