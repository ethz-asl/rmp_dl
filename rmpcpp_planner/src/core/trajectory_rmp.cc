...
template <class Space>
const rmpcpp::TrajectoryPointRMP<Space> &rmpcpp::TrajectoryRMP<Space>::previous()
    const {
  if(trajectory_data_.size() == 1) {
    return trajectory_data_[0]; // Throw exception?
  }
  return trajectory_data_[trajectory_data_.size() - 2];
}
