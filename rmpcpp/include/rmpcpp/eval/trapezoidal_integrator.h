...
class TrapezoidalIntegrator {
 public:
  ...
  Vector_q getPos() const { return current_pos_; }

  Vector_q getVel() const { return current_vel_; }
  Vector_q getAcc() const { return last_acc_; }

  Matrix_q getMetric() const { return last_metric_; }

  void getState(Vector_q& pos, Vector_q& vel, Vector_q& acc) const{
    pos = current_pos_;
    vel = current_vel_;
    acc = last_acc_;
  }
